...
app.get("/libro",async(req,res)=>{
    try{
       ...
       let respuesta = await qy("select persona.alias AS alias , libro.id AS id ,libro.nombre as nombre, libro.descripcion as descripcion, libro.categoria_id AS categoria_id, libro.persona_id AS persona_id from libro LEFT JOIN persona ON persona.id = libro.persona_id");
...
app.get ('/persona/:id/libro', async (req, res) =>{

    let id= [req.params.id];
    try{

        let respuesta = await qy("SELECT * FROM persona WHERE id = ?",id);
        ...
        let consulta = await qy("SELECT * FROM libro WHERE persona_id = ?",id);

        if(consulta.length==0){
            throw new Error ("La persona no tiene libros asociados");
        }
        ...
        res.status(200).send(consulta);
            ...
            if(error.message!=="La persona no existe" && error.message!== "La persona no tiene libros asociados"){
                res.status (413).send ({"error": "error inesperado"});
            }
